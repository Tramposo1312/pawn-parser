...
func (p *Parser) parseCharLiteral() ast.Expression {
	...
	charLiteral := p.curToken.Literal[1 : len(p.curToken.Literal)-1]

	if len(charLiteral) != 1 {
		msg := fmt.Sprintf("invalid char literal: %q", p.curToken.Literal)
		p.errors = append(p.errors, msg)
		return nil
	}

	lit.Value = charLiteral[0]
	return lit
}
...
func (p *Parser) parseFunctionDefinition() *ast.FunctionDefinition {
	fun := &ast.FunctionDefinition{Token: p.curToken}
